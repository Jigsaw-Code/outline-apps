...
func TestPacketConn_ReadFrom_LastRemainingPacket(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	mockDefaultConn := newMockPacketConn(t, "10.0.0.1", "127.0.0.1")
	defaultListener := &mockPacketListener{conn: mockDefaultConn}

	iptableListener, err := NewPacketListener(nil, defaultListener) // No specific table routes, only default
	require.NoError(t, err, "NewPacketListener failed")

	conn, err := iptableListener.ListenPacket(ctx)
	require.NoError(t, err, "ListenPacket failed")
	defer conn.Close()

	originalPacketData := []byte("This is a long test packet data, much longer than 10 bytes.")
	senderAddr := mockDefaultConn.addr

	// Send packet and read first part
	mockDefaultConn.readChan <- originalPacketData

	// Read first part
	readBuf1 := make([]byte, 10)
	n1, addr1, err1 := conn.ReadFrom(readBuf1)

	require.NoError(t, err1, "First ReadFrom failed")
	assert.Equal(t, 10, n1, "First ReadFrom should read 10 bytes")
	assert.Equal(t, senderAddr.String(), addr1.String(), "First ReadFrom address mismatch")
	assert.Equal(t, originalPacketData[:10], readBuf1, "First ReadFrom data mismatch")

	// Read second part
	readBuf2 := make([]byte, 1024)
	n2, addr2, err2 := conn.ReadFrom(readBuf2)

	require.NoError(t, err2, "Second ReadFrom failed")
	expectedRemainingLength := len(originalPacketData) - n1
	assert.Equal(t, expectedRemainingLength, n2, "Second ReadFrom should read remaining bytes")
	assert.Equal(t, senderAddr.String(), addr2.String(), "Second ReadFrom address mismatch, should be same as first")
	assert.Equal(t, originalPacketData[n1:n1+n2], readBuf2[:n2], "Second ReadFrom data mismatch")

	// Ensure lastRemainingPacket is now empty
	anotherPacketData := []byte("Another packet.")
	mockDefaultConn.readChan <- anotherPacketData

	readBuf3 := make([]byte, 1024)
	n3, addr3, err3 := conn.ReadFrom(readBuf3)

	// Assert third read of new packet
	require.NoError(t, err3, "Third ReadFrom failed")
	assert.Equal(t, len(anotherPacketData), n3, "Third ReadFrom should read length of new packet")
	assert.Equal(t, senderAddr.String(), addr3.String(), "Third ReadFrom address mismatch (should be from default conn's sender)")
	assert.Equal(t, anotherPacketData, readBuf3[:n3], "Third ReadFrom data mismatch")
}
