...
import {VpnApi, TunnelStatus, TunnelConfig} from './vpn';
...
export class FakeVpnApi implements VpnApi {
  ...
  async start(_id: string, _name: string, config: TunnelConfig): Promise<void> {
    if (this.running) {
      return;
    }

    const host = config.transport.getHost();
