...
export interface Language {
  ...
  name: string;
  dir: 'ltr' | 'rtl';
  // The locale ID for the support site at https://support.getoutline.org/.
  supportId?: string;
}
...
export function findLanguageMatch(
  ...
  while (parts.length) {
    ...
    const lowerCaseLanguages: Languages = Object.fromEntries(
    ...
    if (!matchFunction && lowerCaseLanguages[lowerJoined]) {
      return lowerCaseLanguages[lowerJoined];
    }

    if (matchFunction) {
      const match = matchFunction(targetCode, joined);
      if (match) {
        return match;
      }
    }
...
export function findMatch(
  ...
  find: (languageCode: string, languageCodeVariant: string) => Language
): Language {
  for (const languageCode of languageCodes) {
    const match = findLanguageMatch(languageCode, {}, find);
    if (match) {
      return match;
    }
  }
