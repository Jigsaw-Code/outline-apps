...
!macro customInstall
  ...
  ${If} ${RunningX64}
    File /r "${PROJECT_DIR}\src\electron\windows\third_party\tap-windows6\bin\amd64\*"
  ${Else}
    File /r "${PROJECT_DIR}\src\electron\windows\third_party\tap-windows6\bin\i386\*"
  ${EndIf}
  SetOutPath -
  File "${PROJECT_DIR}\src\electron\windows\add_tap_device.bat"
  File "${PROJECT_DIR}\src\electron\windows\find_tap_device_name.bat"

  ; OutlineService files, stopping the service first in case it's still running.
  nsExec::Exec "net stop OutlineService"
  File "${PROJECT_DIR}\src\electron\windows\tools\OutlineService\OutlineService\bin\OutlineService.exe"
  File "${PROJECT_DIR}\src\electron\windows\tools\smartdnsblock\bin\smartdnsblock.exe"
  File "${PROJECT_DIR}\src\electron\windows\third_party\newtonsoft\Newtonsoft.Json.dll"
  File "${PROJECT_DIR}\src\electron\windows\install_windows_service.bat"
