...
!macro customInstall
  ...
  ${EndIf}
  SetOutPath -
  File "${PROJECT_DIR}\src\electron\windows\tools\tap_device\add_tap_device.bat"
  File "${PROJECT_DIR}\src\electron\windows\tools\tap_device\find_tap_device_name\main.bat"

  ; OutlineService files, stopping the service first in case it's still running.
  nsExec::Exec "net stop OutlineService"
  File "${PROJECT_DIR}\src\electron\windows\tools\install_windows_service\main.bat"
  File "${PROJECT_DIR}\src\electron\windows\tools\install_windows_service\OutlineService\bin\OutlineService.exe"
  File "${PROJECT_DIR}\src\electron\windows\tools\smart_dns_block\bin\smartdnsblock.exe"
