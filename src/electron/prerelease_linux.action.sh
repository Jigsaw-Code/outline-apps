...
function usage () {
  ...
  echo 1>&2
  echo "Examples:" 1>&2
  echo "Pushes a release candidate of Linux version 1.2.3 to 10% of users listening on the beta channel" 1>&2
...
electron-builder \
  ...
  --config.extraMetadata.version=$(scripts/semantic_version.sh "${VERSION}" linux prerelease) \
