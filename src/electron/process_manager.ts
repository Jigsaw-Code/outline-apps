...
export class ConnectionMediator {
  // not passed to the constructor because no notification for successful launch.
  ...
  static newInstance(config: cordova.plugins.outline.ServerConfig, isAutoConnect: boolean) {
    return RoutingService.getInstanceAndStart(config.host || '', isAutoConnect).then((routing) => {
      return new ConnectionMediator(routing, config);
    });
  }

  private constructor(
      private readonly routing: RoutingService, config: cordova.plugins.outline.ServerConfig) {
    const exits = [
