...
class AppDelegate: UIResponder, UIApplicationDelegate {
    func application(_: UIApplication, didFinishLaunchingWithOptions _: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        let appKitController = loadAppKitIntegrationFramework2()
        appKitController.hello()
        shouldLaunchMainApp { shouldLaunch in
            defer {
                DDLogInfo("Exiting launcher...")
                appKitController.terminate()
            }
            if !shouldLaunch {
            ...
            //appKitController.loadMainApp(launcherBundleId)
        }
        return true
    }
...
func loadAppKitIntegrationFramework2() -> NSObject {
