...
public class VpnTunnel {
  ...
  /**
   * Connects a tunnel between a proxy server and the VPN TUN interface.
   *
   ...
   */
  public synchronized void connectTunnel(final proxy.Client client, boolean isUdpEnabled) throws Exception {
    LOG.info("Connecting the tunnel.");
    if (client == null) {
      throw new IllegalArgumentException("Must provide a Proxy client.");
    }
    if (tunFd == null) {
    ...
    tunnel = Tun2socks.connectProxyTunnel(tunFd.getFd(), client, isUdpEnabled);
  }
