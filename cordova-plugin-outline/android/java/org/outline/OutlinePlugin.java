...
public class OutlinePlugin extends CordovaPlugin {
    ...
    cordova.getThreadPool().execute(() -> {
      try {
        ...
        if (Action.START.is(action)) {
          ...
          final JSONObject policy = args.getJSONObject(2);
          int errorCode = startVpnTunnel(tunnelId, config, policy);
  ...
  private int startVpnTunnel(final String tunnelId, final JSONObject config, final JSONObject policy) throws Exception {
    LOG.info(String.format(Locale.ROOT, "Starting VPN tunnel %s", tunnelId));
    final TunnelConfig tunnelConfig;
    try {
      tunnelConfig = VpnTunnelService.makeTunnelConfig(tunnelId, config, policy);
    } catch (Exception e) {
