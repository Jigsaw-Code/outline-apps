...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                                   \
  ...
  } while ((void)0, false && static_cast<bool>(!!(__VA_ARGS__)))  // the expression here is never
                                                                  // evaluated at runtime but it
                                                                  // forces the compiler to give it
                                                                  // a look
...
struct NullGenerator : IGenerator<T> {
  auto get(size_t) const -> T override { CATCH_INTERNAL_ERROR("A Null Generator is always empty"); }
};
...
class GeneratorRandomiser : public IGenerator<T> {
 ...
 public:
  ...
  auto get(size_t index) const -> T override { return m_baseGenerator[m_indices[index]]; }
};
...
class IndexTracker : public TrackerBase {
 ...
 public:
  IndexTracker(NameAndLocation const& nameAndLocation, TrackerContext& ctx, ITracker* parent,
               int size);

  bool isIndexTracker() const override;
  void close() override;

  static IndexTracker& acquire(TrackerContext& ctx, NameAndLocation const& nameAndLocation,
                               int size);
...
void Approx::setMargin(double margin) {
  CATCH_ENFORCE(margin >= 0, "Invalid Approx::margin: "
                                 << margin << '.' << " Approx::Margin has to be non-negative.");
  m_margin = margin;
}
...
clara::Parser makeCommandLineParser(ConfigData& config) {
  ...
  auto cli =
      ...
      Opt(config.benchmarkResolutionMultiple, "multiplier")["--benchmark-resolution-multiple"](
...
void TrackerBase::close() {
  ...
  switch (m_runState) {
    ...
    case ExecutingChildren:
      if (m_children.empty() || m_children.back()->isComplete()) m_runState = CompletedSuccessfully;
      break;
...
IndexTracker& IndexTracker::acquire(TrackerContext& ctx, NameAndLocation const& nameAndLocation,
                                    int size) {
...
std::string StringMaker<float>::convert(float value) { return fpToString(value, 5) + 'f'; }
...
void ConsoleReporter::benchmarkStarting(BenchmarkInfo const& info) {
  ...
  auto nameCol =
      Column(info.name).width(static_cast<std::size_t>(m_tablePrinter->columnInfos()[0].width - 2));
