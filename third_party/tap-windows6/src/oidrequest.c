...
VOID DBG_PRINT_OID_NAME(__in NDIS_OID Oid) {
  PCHAR oidName = NULL;

  switch (Oid) {

#undef MAKECASE
#define MAKECASE(oidx)    \
  case oidx:              \
    oidName = #oidx "\n"; \
    break;
...
NDIS_STATUS
tapSetMulticastList(__in PTAP_ADAPTER_CONTEXT Adapter, __in PNDIS_OID_REQUEST OidRequest) {
  ...
  do {
    ...
    NdisZeroMemory(Adapter->MCList, TAP_MAX_MCAST_LIST * MACADDR_SIZE);

    NdisMoveMemory(Adapter->MCList, OidRequest->DATA.SET_INFORMATION.InformationBuffer,
                   OidRequest->DATA.SET_INFORMATION.InformationBufferLength);
...
NDIS_STATUS
tapSetPacketFilter(__in PTAP_ADAPTER_CONTEXT Adapter, __in ULONG PacketFilter) {
  NDIS_STATUS status = NDIS_STATUS_SUCCESS;
...
NDIS_STATUS
AdapterSetPowerD0(__in PTAP_ADAPTER_CONTEXT Adapter)
/*++
Routine Description:
...
NDIS_STATUS
AdapterSetPowerLow(__in PTAP_ADAPTER_CONTEXT Adapter, __in NDIS_DEVICE_POWER_STATE PowerState)
/*++
Routine Description:
...
NDIS_STATUS
tapSetInformation(__in PTAP_ADAPTER_CONTEXT Adapter, __in PNDIS_OID_REQUEST OidRequest)
/*++
...
--*/
{
  ...
  switch (OidRequest->DATA.SET_INFORMATION.Oid) {
    ...
    case OID_GEN_CURRENT_PACKET_FILTER:
      ...
      status = tapSetPacketFilter(Adapter,
    ...
    case OID_PNP_SET_POWER: {
      ...
      if (OidRequest->DATA.SET_INFORMATION.InformationBufferLength <
        ...
        PowerState =
            *(PNDIS_DEVICE_POWER_STATE UNALIGNED)OidRequest->DATA.SET_INFORMATION.InformationBuffer;
        OidRequest->DATA.SET_INFORMATION.BytesRead = sizeof(NDIS_DEVICE_POWER_STATE);

        if (PowerState < NdisDeviceStateD0 || PowerState > NdisDeviceStateD3) {
...
NDIS_STATUS
tapQueryInformation(__in PTAP_ADAPTER_CONTEXT Adapter, __in PNDIS_OID_REQUEST OidRequest)
/*++
...
--*/
{
  ...
  switch (OidRequest->DATA.QUERY_INFORMATION.Oid) {
    ...
    case OID_GEN_XMIT_ERROR:
      ulInfo =
          (ULONG)(Adapter->TxAbortExcessCollisions + Adapter->TxDmaUnderrun + Adapter->TxLostCRS +
                  Adapter->TxLateCollisions + Adapter->TransmitFailuresOther);
      pInfo = &ulInfo;
      break;

    case OID_GEN_RCV_ERROR:
      ulInfo = (ULONG)(Adapter->RxCrcErrors + Adapter->RxAlignmentErrors +
                       Adapter->RxDmaOverrunErrors + Adapter->RxRuntErrors);
      pInfo = &ulInfo;
      break;
    ...
    case OID_GEN_XMIT_OK:
      ulInfo64 =
          Adapter->FramesTxBroadcast + Adapter->FramesTxMulticast + Adapter->FramesTxDirected;
      pInfo = &ulInfo64;
    ...
    case OID_GEN_RCV_OK:
      ulInfo64 =
          Adapter->FramesRxBroadcast + Adapter->FramesRxMulticast + Adapter->FramesRxDirected;
    ...
    case OID_GEN_STATISTICS:
      ...
      if (OidRequest->DATA.QUERY_INFORMATION.InformationBufferLength <
        ...
        Statistics->ifHCInOctets =
            Adapter->BytesRxDirected + Adapter->BytesRxMulticast + Adapter->BytesRxBroadcast;

        Statistics->ifHCInUcastOctets = Adapter->BytesRxDirected;

        Statistics->ifHCInMulticastOctets = Adapter->BytesRxMulticast;

        Statistics->ifHCInBroadcastOctets = Adapter->BytesRxBroadcast;
        ...
        /* Errors in */
        Statistics->ifInErrors = Adapter->RxCrcErrors + Adapter->RxAlignmentErrors +
                                 Adapter->RxDmaOverrunErrors + Adapter->RxRuntErrors;
        ...
        Statistics->ifHCOutOctets =
            Adapter->BytesTxDirected + Adapter->BytesTxMulticast + Adapter->BytesTxBroadcast;

        Statistics->ifHCOutUcastOctets = Adapter->BytesTxDirected;

        Statistics->ifHCOutMulticastOctets = Adapter->BytesTxMulticast;

        Statistics->ifHCOutBroadcastOctets = Adapter->BytesTxBroadcast;
        ...
        /* Errors out */
        Statistics->ifOutErrors = Adapter->TxAbortExcessCollisions + Adapter->TxDmaUnderrun +
                                  Adapter->TxLostCRS + Adapter->TxLateCollisions +
  ...
  if (status == NDIS_STATUS_SUCCESS) {
    ...
    if (ulInfoLen <= OidRequest->DATA.QUERY_INFORMATION.InformationBufferLength) {
      if (pInfo) {
        ...
        NdisMoveMemory(OidRequest->DATA.QUERY_INFORMATION.InformationBuffer, pInfo, ulInfoLen);
      }
...
NDIS_STATUS
AdapterOidRequest(__in NDIS_HANDLE MiniportAdapterContext, __in PNDIS_OID_REQUEST OidRequest)
/*++
...
VOID AdapterCancelOidRequest(__in NDIS_HANDLE MiniportAdapterContext, __in PVOID RequestId) {
  PTAP_ADAPTER_CONTEXT adapter = (PTAP_ADAPTER_CONTEXT)MiniportAdapterContext;
