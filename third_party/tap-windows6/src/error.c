...
VOID MyDebugInit(unsigned int bufsiz) {
...
#endif
        va_start(args, format);
        status = RtlStringCchVPrintfExA(g_Debug.text + g_Debug.out, remaining, &end, NULL,
                                        STRSAFE_NO_TRUNCATION | STRSAFE_IGNORE_NULLS, format, args);
        va_end(args);
        va_start(args, format);
...
GetDebugLine(__in char *buf, __in const int len) {
  ...
  if (g_Debug.text && g_Debug.capacity > 0) {
    ...
    if (owned) {
      ...
      if (g_Debug.error ||
          NewlineExists(g_Debug.text + g_Debug.in, (int)g_Debug.out - (int)g_Debug.in)) {
...
VOID PrMac(const MACADDR mac) {
  DEBUGP(("%x:%x:%x:%x:%x:%x", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]));
}

VOID PrIP(IPADDR ip_addr) {
  const unsigned char *ip = (const unsigned char *)&ip_addr;

  DEBUGP(("%d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]));
}

const char *PrIPProto(int proto) {
  switch (proto) {
    case IPPROTO_UDP:
      return "UDP";
...
VOID DumpARP(const char *prefix, const ARP_PACKET *arp) {
  DEBUGP(("%s ARP src=", prefix));
  PrMac(arp->m_MAC_Source);
  DEBUGP((" dest="));
  PrMac(arp->m_MAC_Destination);
  DEBUGP((" OP=0x%04x", (int)ntohs(arp->m_ARP_Operation)));
  DEBUGP((" M=0x%04x(%d)", (int)ntohs(arp->m_MAC_AddressType), (int)arp->m_MAC_AddressSize));
  DEBUGP((" P=0x%04x(%d)", (int)ntohs(arp->m_PROTO_AddressType), (int)arp->m_PROTO_AddressSize));
...
VOID DumpPacket2(__in const char *prefix, __in const ETH_HEADER *eth,
                 __in const unsigned char *data, __in unsigned int len) {
  struct ethpayload *ep = (struct ethpayload *)MemAlloc(sizeof(struct ethpayload), TRUE);
  if (ep) {
    if (len > DEFAULT_PACKET_LOOKAHEAD) len = DEFAULT_PACKET_LOOKAHEAD;
    ep->eth = *eth;
...
VOID DumpPacket(__in const char *prefix, __in const unsigned char *data, __in unsigned int len) {
  ...
  // IPv4 packet?
  if (len >= (sizeof(IPHDR) + sizeof(ETH_HEADER)) && eth->proto == htons(ETH_P_IP) &&
    ...
    // TCP packet?
    if (ip->protocol == IPPROTO_TCP && blen - hlen >= (sizeof(TCPHDR))) {
    ...
    // UDP packet?
    else if ((ntohs(ip->frag_off) & IP_OFFMASK) == 0 && ip->protocol == IPPROTO_UDP &&
      ...
      if ((udp->dest == htons(BOOTPC_PORT) || udp->dest == htons(BOOTPS_PORT)) &&
        ...
        const DHCP *dhcp = (DHCP *)(data + hlen + sizeof(ETH_HEADER) + sizeof(UDPHDR));

        int optlen = len - sizeof(ETH_HEADER) - hlen - sizeof(UDPHDR) - sizeof(DHCP);
